...
def tampilkan_soal(indeks, skor):
    ...
    kuis_frame.pack()
    lbl_soal.config(text=soal_kuis[indeks]["soal"])
...
# Inisialisasi GUI
root = tk.Tk()
root.geometry("500x400")  # Mengatur ukuran window
root.title("Aplikasi Kuis")  # Judul aplikasi
...
tk.Button(main_menu_frame, text="Tambah Soal", command=lambda: tambah_soal_frame.pack()).pack()
...
input_jawaban.pack()
tk.Button(tambah_soal_frame, text="Tambah Soal", command=tambah_soal).pack()
tk.Label(tambah_soal_frame, text="Indeks Soal untuk Dihapus:").pack()
input_indeks = tk.Entry(tambah_soal_frame)
input_indeks.pack()
tk.Button(tambah_soal_frame, text="Hapus Soal", command=hapus_soal).pack()
tk.Button(tambah_soal_frame, text="Kembali", command=lambda: tambah_soal_frame.pack_forget()).pack()

# Frame Kuis
kuis_frame = tk.Frame(root)
lbl_soal = tk.Label(kuis_frame, text="")
lbl_soal.pack()
